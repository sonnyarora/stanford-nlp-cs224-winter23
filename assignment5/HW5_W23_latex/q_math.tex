...
\begin{subparts}
    ...
    \ifans{The $\alpha_i$ are the output of a softmax over the $k_i q$, so they sum to 1 and 
        have values between 0 and 1. Interpreting each $\alpha_i$ as representing the probability
        of category $i$ then makes them a categorical probability distribution.}
    ...
    \ifans{The query $q$ must have a significantly larger dot product with one of the keys
        $k_i$ than with the other $k_j$. Specifically,
        \begin{align*}
            \alpha_j &>> \sum_{i \ne j} \alpha_i\\      
            \implies \dfrac{\exp(k_i^T q)}{\sum_{w=1}^n \exp(k_j^T q)} &>> \sum_{i \ne j}\dfrac{exp(k_j^T q)}{\sum_{w=1}^n \exp(k_w^T q)}\\
            \implies \exp(k_i^T q) &>> \sum_{i \ne j} \exp(k_j^T q)\\
        \end{align*}

        As $\exp(k_j^Tq) > 0$, it follows that for all $i \ne j$, $\exp(k_i^Tq) >> \exp(k_j^T q)$. Therefore,
        as $\exp$ is monotonic, $k_i^T q$ is larger than $k_j^T q$ for $i \ne j$. 

    }
    \subpart[1] Under the conditions you gave in (ii),  \textbf{describe} the output $c$. 
    
    \ifans{As $\sum \alpha_w = 1$, $\alpha_j >> \sum_{i \ne j} \alpha_j$ implies $c \approx v_i\alpha_i.$}

    \subpart[1] \textbf{Explain} (in two sentences or fewer) what your answer to (ii) and (iii) means intuitively. \\
\ifans{When a query $q$ matches (according to dot product similarity) a key $k_i$ significantly better than the other keys
$k_j$, the output is approximately the value $v_i$. This is similar to an exact query-key match system.}
...
\ifans{Let $A$ be the matrix with column vectors the $a_i$. Let $M = 2AA^T$.
\begin{align*}
    Ms &= 2AA^T \cdot \dfrac{1}{2}(v_a + v_b)\\
        &= AA^T \cdot (v_a) && \text{ As } v_b \cdot a_i = 0 \text{ for all } a_i\\
        &= A \cdot c\\ 
...
\ifans{Choose $q = \sum_{j=1}^n d_j k_j$ where $d_j \in \mathbb{R}$ and such that 
$d_a = d_b$, $d_a, d_b >> d_j$ where $j \ne a,b$. This gives:
\begin{align*}
    \dfrac{exp(k_a^T q)}{\sum_{j=1}^n \exp(k_j^T q)} &\approx \dfrac{\exp(k_a^T q)}{\exp(k_a^T q) + \exp(k_b^T q)}\\
            &= \dfrac{\exp(k_a^T q)}{2 \exp(k_a^T q)}\\
            &= 1/2
\end{align*}
}
\end{subparts}
...
\ifans{Let $g(\overrightarrow{k}) = c$ where $\overrightarrow{{k}}$ is the vector random variable of 
keys $(k_1, \cdots, k_n)$. Use $q = \sum_{j=1}^n d_j k_j$ where $d_a = d_b$ and $d_a >> d_j$ for
$j \ne a,b$. A rough explanation: Using a first order Taylor approximation for the expected value,
\begin{align*}
    E[g(\overrightarrow{k})] &\approx g(\mu_1, \cdots \mu_n)\\
        &\approx \dfrac{1}{2}(v_a + v_b) && \text{By a similar argument to part bii)}
\end{align*}
Let $\theta$ be the tuple of parameters for the distributions of the $k_i$. Also, let $g_i'(\theta)$ be the partial with respect to the $i_th$ random variable of the $\overrightarrow{k}$. Using a first order Taylor approximation to the variance,
\begin{align*}
    Var(\overrightarrow{k}) &= \sum_{i=1}^k[g_i'(\theta)]^2 Var_\theta k_i + 2 \sum_{i > j}g_i'(\theta)g_j'(\theta)Cov_\theta(k_i, k_j)\\
                    &= \sum_{i=1}^k [g_i'(\theta)]^2 Var_\theta k_i
\end{align*}
As $\alpha \rightarrow 0$, $Var(\overrightarrow{k}) \rightarrow 0$.
}
