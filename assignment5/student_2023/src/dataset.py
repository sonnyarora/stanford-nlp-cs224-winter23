...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        document = [self.stoi[s] for s in document]
        truncate_length = random.randint(4,int(self.block_size*(7/8))) 
        truncated_doc = document[:truncate_length]
        masked_content_len = binom.rvs(truncate_length,(1/4))
        prefix_len = int((truncate_length - masked_content_len)/2)
        ...
        pad_len = self.block_size - len(truncated_doc) - 2
        #breakpoint()
        masked_string = np.pad((prefix + [self.stoi[self.MASK_CHAR]] + suffix + [self.stoi[self.MASK_CHAR]] + masked_content), (0, pad_len), 'constant', constant_values=(0,self.stoi[self.PAD_CHAR]))
        x = torch.tensor(masked_string[:-1], dtype=torch.long)
        y = torch.tensor(masked_string[1:], dtype=torch.long)
        return x, y
