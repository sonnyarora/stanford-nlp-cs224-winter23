...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.hidden_to_logits_bias = nn.Parameter(torch.ones(1,n_classes))
    ...
    def embedding_lookup(self, w):
        ...
        x = self.embeddings[w].view(w.shape[0], w.shape[1]*self.embeddings.shape[1])
        #x = [torch.flatten(torch.tensor([self.embeddings[feature] for feature in features])) for features in w]
        #x = torch.tensor(x)
    ...
    def forward(self, w):
        ...
        h = self.relu(self.embedding_lookup(w).matmul(self.embed_to_hidden_weight) + self.embed_to_hidden_bias)
        h_dropout = self.dropout(h)
        logits = h_dropout.matmul(self.hidden_to_logits_weight) + self.hidden_to_logits_bias
