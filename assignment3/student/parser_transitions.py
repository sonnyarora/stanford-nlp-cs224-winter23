...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            elem = self.stack.pop(-2)
            self.dependencies.append((self.stack[-1],elem))
        elif transition == "RA":
...
def minibatch_parse(sentences, model, batch_size):
    ...
    unfinished_parses = partial_parses[:]
    # Chunk Function from https://stackoverflow.com/questions/312443/how-do-i-split-a-list-into-equally-sized-chunks/22045226#22045226
    #def chunks(lst, n):
    ...
    while unfinished_parses != []:
        ...
        for i in range(0, len(unfinished_parses), batch_size):
            end = min(i+batch_size,len(unfinished_parses)) 
            batch = unfinished_parses[i:end]
            transitions = model.predict(batch)
            for j in range(i, end):
                unfinished_parses[j].parse_step(transitions[j-i])
                if unfinished_parses[j].buffer == [] and len(unfinished_parses[j].stack) == 1:
                    parses_to_remove.append(j)
        for idx_to_remove in sorted(parses_to_remove, reverse=True):
            unfinished_parses.pop(idx_to_remove)
