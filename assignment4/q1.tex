...
\end{lstlisting}
    ...
    \ifans{One advantage of dot product attention over multiplicative attention is that it is less expensive to compute. One disadvantage is that it is less general. }
    \subpart[2] Explain one advantage and one disadvantage of \textit{additive attention} compared to multiplicative attention.

    \ifans{One advantage of dot product attention over additive attention is that it is more interpretable - you can see which elements of the vector $h_i$ are picked out by $s_i$. One advantage of the additive attention is that a non-linear function (tanh) is applied to the vector components before projection. This can potentially allow some non-linear interactions between components of each vector.}
    \end{subparts}
