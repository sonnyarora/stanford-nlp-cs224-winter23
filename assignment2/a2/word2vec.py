...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow.
    #breakpoint()
    U_T_product_v_c = outsideVectors.dot(centerWordVec)  
    y_hat = softmax(U_T_product_v_c)
...
def negSamplingLossAndGradient(
    ...
    #negSampleWordIndicesDict = defaultdict(lambda: 0)
    #for idx in negSampleWordIndices:
    #    negSampleWordIndicesDict[idx] += 1
    U = outsideVectors[indices,]
    U[1:,] = -U[1:,]
    one_minus_sigmoid_U = 1 - sigmoid(U.dot(centerWordVec))
    loss = np.sum(-np.log(sigmoid(U.dot(centerWordVec))))
    gradCenterVec = -np.sum(U.transpose() * one_minus_sigmoid_U, axis=1)
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx] = -centerWordVec*(1-sigmoid(outsideVectors[outsideWordIdx].dot(centerWordVec)))
    for idx in negSampleWordIndices:
        gradOutsideVecs[idx,] += centerWordVec * (1-sigmoid(-outsideVectors[idx].dot(centerWordVec)))
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        loss_term, gradCenterVecs_term, gradOutsideVectors_term = word2vecLossAndGradient(centerWordVec, word2Ind[outsideWord], outsideVectors, dataset)
        loss += loss_term
        gradCenterVecs[word2Ind[currentCenterWord],] += gradCenterVecs_term
        gradOutsideVectors += gradOutsideVectors_term
        #breakpoint()
    #loss_and_gradient_arr = [list(
