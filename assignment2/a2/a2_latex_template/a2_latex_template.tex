...
\newcommand{\hmwkAuthorName}{Sonny Arora} % Your name
...
%	Custom Commands
%----------------------------------------------------------------------------------------
\newcommand\partialJ[1]{\dfrac{\partial J}{\partial #1}}
\newcommand\derivative[2]{\dfrac{\mathrm{d} #1}{\mathrm{d} #2}}
%----------------------------------------------------------------------------------------
%	TITLE PAGE
%----------------------------------------------------------------------------------------
\title{
...
\begin{shaded}
\begin{answer}
As $y_w$ is a one-hot encoded vector with a $1$ when $w=o$, after taking the sum, you are only left with the term in equation (2).
\end{answer}
\end{shaded}
...
\begin{enumerate}[label=(\roman*)]
    \item 
    Compute the partial derivative ofm  $\bm J_{\text{naive-softmax}}(\bm v_c, o, \bm U)$ with respect to $\bm v_c$. \ul{Please write your answer in terms of $\bm y$, $\hat{\bm y}$, $\bm U$, and show your work to receive full credit}.
    \begin{itemize} 
...
\begin{answer}
    \begin{enumerate}[label=(\roman*)]
        \item From the lecture notes, :
        \begin{align*}
            \nabla_{v_c} J &= -u_{o} + \sum_{x=1}^n P_{U,V}(x \mid c) u_x\\
                        &= -u_{o} + \sum_{x=1}^n \hat{y}_x u_x\\
                        &= -u_{o} + U \cdot \hat{y}\\ 
                        &= -U \cdot y + U \cdot \hat{y}
        \end{align*}
        \item When $y - \hat{y}$ is in the null space of $U$. 
        \item The first term can also be written as $u_o$. Subtracting $\alpha u_o$ from $v_c$, makes $v_c$ more similar to $u_o$, thus,
         increasing 
    \end{enumerate}
...
\begin{shaded}
\begin{answer}
Let $J = J_{naive-softmax}(v_c, o, U)$. 
\begin{align*}
    J &= -\log \dfrac{\exp{u_o^{T} v_c}}{\sum_{w \in Vocab} \exp{u_w^T v_c}}\\
        &= - u_o^{T} v_c + \log(\sum_{w \in Vocab} \exp{u_w^T v_c})\\ 
    \dfrac{\partial J}{\partial u_o} &= - v_c + \left(\dfrac{1}{\sum_{w \in Vocab} \exp (u_w^T v_c)} \cdot \exp (u_o^T v_c) \cdot v_{c_i}\right)_{i}\\
        &= -v_c + \dfrac{\exp (u_o^T v_c)}{\sum_{w \in Vocab} \exp (u_w^T v_c)} \cdot v_c\\
        &= -v_c + \hat{y}_o \cdot v_c\\
    \dfrac{\partial J}{\partial u_w} &= 0 + \left(\dfrac{1}{\sum_{w \in Vocab} \exp (u_w^T v_c)} \cdot \exp (u_w^T v_c) \cdot v_{c_i}\right)_{i}\\
        &= \hat{y}_w \cdot v_c
\end{align*}
\end{answer}
\end{shaded}
...
\begin{shaded}
\begin{answer}
The columns of $\dfrac{\partial J}{\partial U}$ are. If $i \ne o$:
\[\partialJ{u_i} = \hat{y}_i \cdot v_c \]
And:
\[\partialJ{u_o} = -v_c + \hat{y}_o \cdot v_c\] 
\end{answer}
\end{shaded}
...
\begin{shaded}
\begin{answer}
As $0 < \alpha < 1$, $\alpha x < x$ if $x >0$ and $\alpha x > x$ if $x < 0$. Therefore, for $x > 0$
\begin{align*}
    f(x) &= \max(\alpha x, x)\\
        &= x\\
    \derivative{f}{x} &= 1\\
...
For $x < 0$:
\begin{align*}
    \derivative{f}{x} &= \derivative{(\alpha x)}{x}\\
        &= \alpha\\
\end{align*}
\end{answer}
\end{shaded}
...
\begin{answer}
\begin{align*}
   \derivative{\sigma}{x} &= \derivative{}{x}\dfrac{e^x}{e^x +1}\\ 
                &= \dfrac{e^x}{e^x +1} - \dfrac{e^x}{(e^x +1)^2} \cdot e^x\\
                &= \sigma(x) - \sigma^2(x)\\
                &= \sigma(x)(1 - \sigma(x))
\end{align*}
\end{answer}
...
\begin{answer}
    \begin{enumerate}[label=(\roman*)]
        \item 
            \begin{align*}
                \partialJ{v_c} &= -\dfrac{1}{\sigma(u_{o}^T v_c)}(1 - \sigma(u_{o}^Tv_c))\cdot \sigma(u_{o}^T v_c) \cdot u_{o} - \left(\sum_{s=1}^K \dfrac{1}{\sigma(-u_{w_s}^T v_c)}\sigma(-u_{w_s}^T v_c) (1 - \sigma(-u_{w_s}^T v_c)) \cdot (-u_{w_{s_i}})\right)_i\\
                        &= -(1-\sigma(u_o^Tv_c))^T \cdot u_o + \sum_{s=1}^K (1-\sigma(-u_{w_s}^T v_c))^T \cdot -u_{w_s}\\ 
                \partialJ{u_o} &= -\dfrac{1}{\sigma(u_o^T v_c)}(\sigma(u_o^T v_c))(1 - \sigma(u_o^T v_c))\cdot (v_c)\\
                        &= -(1-\sigma(u_o^Tv_c))\\
                \partialJ{u_{w_s}} &= -\dfrac{1}{\sigma(-u_{w_s}^Tv_c)}( \sigma(-u_{w_s}^Tv_c) )(1-\sigma(-u_{w_s}^T v_c))\cdot (-v_c)\\
                    &= -(1-\sigma(-u_{w_s}^T v_c))^T \cdot(-v_c)\\
            \end{align*}
        \item 
        \[{\bf 1} - \sigma(U_{o,\{w_1,\dots,w_K\}}^Tv_c)\]
        \item Compared to naive-softmax, we do not have to take a sum over the entire vocabulary when computing the derivatives.
    \end{enumerate}
\end{answer}
\end{shaded}
...
\begin{answer}
    \begin{align*}
        \partialJ{u_{w_s}} &= \dfrac{\partial}{\partial u_{w_s}}(-\log(\sigma(u_o^Tv_c)))-\dfrac{\partial}{\partial u_{w_s}}\left(\sum_{s=1}^K \log(\sigma(-u_{w_s}^T v_c)\right)\\
                &= 0 + \dfrac{\partial}{\partial u_{w_s}}\sum_{i=1, w_i\ne w_s}^K \log(\sigma(-u_{w_s}^T v_c)) + \dfrac{\partial}{\partial u_{w_s}}\sum_{j=1,w_j=w_s}^K \log(\sigma(-u_{w_s}^T v_c))\\
                &= \sum_{j=1,w_j = w_s}^K -(1-\sigma(-u_s^Tv_c)) \cdot (-v_c)\\
    \end{align*}
\end{answer}
\end{shaded}
